...
(function(a,b){if(typeof define==="function"&&define.amd){define("adsk.core.browserStorageWhitelist",[],b)
}else{a.adsk=a.adsk||{};
a.adsk.BrowserStorageWhitelist=b()
}}(typeof self!=="undefined"?self:this,function(){var c={};
var b=function(j){var l={adskGdprAdvertising:window.adskGdprAdvertising,adskGdprAnalytics:window.adskGdprAnalytics,adskGdprCommunication:window.adskGdprCommunication,adskGdprFeedback:window.adskGdprFeedback,adskGdprTroubleshooting:window.adskGdprTroubleshooting};
var g=function(m){m=m||false;
var n;
if(m){n=function(o,p){var q=l.hasOwnProperty(p)&&typeof l[p]!=="undefined";
return o&&q
}
}else{n=function(p,q){var o=!!l[q];
return p&&o
}
}return Object.keys(l).reduce(n,true)
};
var f=g(true);
var i="nonsensitive";
var h;
var k;
if(typeof localStorage!=="object"||!c.browserstorage||!f){return
}switch(j){case"localStorage":h=window.localStorage;
k=c.browserstorage.local;
break;
case"sessionStorage":h=window.sessionStorage;
k=c.browserstorage.session;
break;
default:throw new Error('Function expects a string of either "localStorage" or "sessionStorage"');
break
}if(k.length===0){return
}var e=g();
Object.keys(h).map(function(n){if(n.indexOf(i)>=0){return
}var m=k.filter(function(q){var p=q.key;
var r=p.indexOf("*");
var o=false;
if(r>=0){p=p.slice(0,r);
o=n.substring(0,p.length)===p
}else{o=n===p
}if(o&&!e&&q.pii){return false
}return o
});
if(m.length===0){h.removeItem(n)
}return
})
};
var a=function(e){c.browserstorage=c.browserstorage||e
};
var d=function(){b("localStorage");
b("sessionStorage");
window.addEventListener("beforeunload",function(e){b("localStorage");
b("sessionStorage")
});
Storage.prototype._getItem=Storage.prototype.getItem;
Storage.prototype.getItem=function(e){b("localStorage");
b("sessionStorage");
return this._getItem(e)
}
};
return{initialize:a,execute:d}
}));
define("adsk.core.privateMode",[],function(){var a=function(){window.hasNoAccessToLocalStorage=false;
if(typeof localStorage==="object"){try{localStorage.setItem("localStorage",1);
localStorage.removeItem("localStorage")
}catch(f){var d=function(k){function i(p,q,r){var o,n;
if(r){o=new Date();
o.setTime(o.getTime()+(r*24*60*60*1000));
n="; expires="+o.toGMTString()
}else{n=""
}document.cookie=p+"="+q+n+"; path=/"
}function m(o){var q=o+"=",n=document.cookie.split(";"),p,r;
for(p=0;
p<n.length;
p++){r=n[p];
...
}function h(n){n=encodeURIComponent(JSON.stringify(n));
if(k=="session"){i(j(),n,365)
}else{i("localStorage",n,365)
}}function g(){if(k=="session"){i(j(),"",365)
}else{i("localStorage","",365)
}}function e(){var n=k=="session"?m(j()):m("localStorage");
return n?JSON.parse(decodeURIComponent(n)):{}
}function j(){if(!window.name){window.name=new Date().getTime()
}return"sessionStorage"+window.name
}var l=e();
return{length:0,clear:function(){l={};
this.length=0;
g()
},getItem:function(n){return l[n]===undefined?null:l[n]
},key:function(o){var p=0;
for(var n in l){if(p==o){return n
}else{p++
}}return null
},removeItem:function(n){delete l[n];
this.length--;
h(l)
},setItem:function(n,o){l[n]=o+"";
this.length++;
h(l)
}}
};
...
window.localStorage.__proto__=c;
window.sessionStorage.__proto__=b;
window.hasNoAccessToLocalStorage=true;
console.warn("Access to LocalStorage is forbidden, using Cookie storage instead")
}}};
return{execute:a}
});
define("adsk.core.possibleNow",["jquery"],function(a){var b={};
b.sendCustomerPreferences=function(c,d){var e=new Date().toISOString();
var f={date:e};
f=a.extend({},f,d);
a.ajax({url:"/services/adsk/c/oxygen/"+c+".json",type:"POST",data:a.param(f),contentType:"application/x-www-form-urlencoded; charset=UTF-8"}).done(function(){console.log("Updated user preferences")
}).fail(function(i,g,h){console.error("Could not update user preferences. Status: "+g+" Message: "+h)
})
};
return b
});
...
adsk.webSDK.utils=adsk.webSDK.utils||{};
adsk.webSDK.utils.getWebInstallParams=function(b,d,i,f,g,a,h){if(!b.installNowUri){return undefined
}var k=b.installNowUri;
var c={serverData:{serverDataString:d.serverDataString,accessId:d.accessId,signature:d.signature},clientData:{selections:[{paramId:"url",optionId:k},{paramId:"productLang",optionId:b.locale},{paramId:"akamai",isSet:true},{paramId:"student",isSet:f},{paramId:"trial",isSet:false},{paramId:"trialMode",isSet:g},{paramId:"skipPI",isSet:false},{paramId:"skipWC",isSet:false},{paramId:"skipWCTools",isSet:false}],params:[{id:"auth",value:"authparam"}]}};
if(i){var e=false;
if(b.eulaId){e=true
}c.clientData.selections.push({paramId:"skipEULA",isSet:e})
}if(a){if(b.productKey){c.clientData.params.push({id:"PK",value:b.productKey})
}}if(h){c.clientData.params.push({id:"SN",value:h})
}var j=adsk.webSDK.utils.getBrowserDownloadUri(b);
if(adsk.webSDK.isWebSDKDirectDownloadUrl&&j){c.clientData.selections.push({paramId:"directUrl",optionId:j})
}return c
};
adsk.webSDK.utils.getBrowserDownloadUri=function(b){var c=RegExp("^.*_\\d\\d1_\\d\\d\\d\\..*$");
var a=b.browserDownloadUris;
if(a==null||a.length==0){return undefined
...
adsk.webSDK.utils.getDownloadNowParams=function(a,c){if(!c||!a||!a.downloadNowUris||a.downloadNowUris.length<1){return undefined
}var b={selections:[{paramId:"url",optionId:a.locale},{paramId:"productLang",optionId:a.locale}]};
return{serverData:c,clientData:JSON.stringify(b)}
};