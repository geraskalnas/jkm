...
$CQ.ajax({url:CQ.shared.HTTP.externalize(a+".segment.js"),dataType:"script",async:true}).complete(function(){CQ_Analytics.SegmentMgr.areSegmentsLoaded=true;
...
if(window.CQ_trackTeasersStats){if(!CQ_Analytics.loadedTeasersStack){CQ_Analytics.loadedTeasersStack=[]
}if(u.segments.length>0){document.getElementById(g).setAttribute("data-wat-content-segment",u.segments[0])
}if(typeof u.path!==undefined){document.getElementById(g).setAttribute("data-wat-content-name",u.path)
...
}if(!CQ_Analytics.UserProfileMgr){CQ_Analytics.UserProfileMgr=function(){this.getDataFromSessionStorage=function(a){if(!window.sessionStorage){return{}
}var c=sessionStorage.getItem(a);
var b={};
if(c!==null){try{b=JSON.parse(c)
}catch(d){}}return b
};
this.updateSessionStore=function(a,c){if(!window.sessionStorage){return false
}var b=this.getDataFromSessionStorage(this.sessionStoreName);
b[a]=c;
sessionStorage.setItem(this.sessionStoreName,JSON.stringify(b))
};
this.clearSessionStore=function(){if(!window.sessionStorage){return false
}sessionStorage.removeItem(this.sessionStoreName)
};
this.getPlatformInfo=function(){var d={};
var c=navigator.appVersion.toLowerCase();
var b=navigator.platform.toLowerCase();
d.isMobile=/mobile|mini|fennec|android|ip(ad|od|hone)/.test(c);
var a="Unknown OS";
if(c.indexOf("win")!=-1){a="Windows"
}if(c.indexOf("mac")!=-1){a="MacOS"
}if(/(iphone|ipod|ipad)/.test(c)){a="IOS"
}if(c.indexOf("x11")!=-1){a="UNIX"
}if(c.indexOf("linux")!=-1||b.indexOf("linux")!=-1){a="Linux"
}if(c.indexOf("android")!=-1){a="Android"
}d.os=a;
return d
};
this.addProperties=function(a){var b;
for(var b in a){this.data[b]=a[b]
}}
};
CQ_Analytics.UserProfileMgr.prototype=new CQ_Analytics.SessionStore();
CQ_Analytics.UserProfileMgr.prototype.STOREKEY="USERPROFILE";
CQ_Analytics.UserProfileMgr.prototype.STORENAME="userprofile";
CQ_Analytics.UserProfileMgr.prototype.sessionStoreName="profile";
CQ_Analytics.UserProfileMgr.prototype.clear=function(){this.data=null
};
CQ_Analytics.UserProfileMgr.prototype.reset=function(){if(this.data!=null){this.data=null;
this.clearSessionStore();
this.fireEvent("update")
}};
CQ_Analytics.UserProfileMgr.prototype.init=function(a){this.data={};
this.setProperties(this.getDataFromSessionStorage(this.sessionStoreName));
this.addProperties(this.getPlatformInfo());
this.initialized=true;
this.fireEvent("initialize",this);
this.fireEvent("update")
};
CQ_Analytics.UserProfileMgr.prototype.setProperties=function(a){this.data={};
for(var b in a){this.data[b]=a[b]
}this.fireEvent("update");
return true
};
CQ_Analytics.UserProfileMgr.prototype.setProperty=function(a,b){this.data[a]=b;
this.updateSessionStore(a,b);
this.fireEvent("update")
};
CQ_Analytics.UserProfileMgr=new CQ_Analytics.UserProfileMgr();
CQ_Analytics.CCM.addListener("configloaded",function(){CQ_Analytics.CCM.register(this)
},CQ_Analytics.UserProfileMgr)
}var adsk=adsk||{};
...
a.updatePersonalizationTags=function(){var c=$("meta[name=personalization-tags]").attr("content");
if(c){b(c.split(/,\s/))
}};
a.updateAssetPersonalizationTags=function(c){if(c){b(c.split(/,/))
}};
a.updateTagsForAsset=function(d){var c;
if(adsk&&adsk.pathCenter&&adsk.pathCenter.tagpath){c=adsk.pathCenter.tagpath+"?path="+d
}else{c="/services/adsk/c/asset-tag-service?path="+d
...
$(function(){adsk.tagCloud.updatePersonalizationTags();
...
var adsk=adsk||{};
adsk.referrerPersonalization=(function(f){f.init=function(m,l){var j=g();
var k=m.getProperty("domain");
var h=m.getProperty("visit");
if((j!=="")&&(location.hostname!==j)){b(m,j)
}else{if((k==="")&&(h==="")){d(m)
}}c(m,l)
};
function g(){var h=window.opener?window.opener.document.referrer:document.referrer;
var j=h.match(/^https?\:\/\/([^\/:?#]+)(?:[\/:?#]|$)/i);
return(j&&j[1])?j[1]:""
}function b(l,h){var k=l.getProperty("domain");
var j=1;
if(h===k){j+=parseInt(l.getProperty("visit"),10)
}else{l.setProperty("domain",h)
}l.setProperty("visit",j)
}function d(h){h.setProperty("domain","");
h.setProperty("visit",0)
}function c(j,h){$.getJSON(h,function(k){if(k&&k.items){a(j,k.items)
}})
}function a(l,h){var k=e();
var j;
if(h instanceof Array){for(j=0;
j<h.length;
j++){if(h[j] in k){l.setProperty(h[j],k[h[j]])
}}}}function e(){var l={};
var h;
if(window.location.search.length>0){var m=window.location.search.substring(1);
var k=m.split("&");
for(var h=0;
h<k.length;
h++){var j=k[h].split("=");
if(j[1]!==undefined){l[decodeURIComponent(j[0])]=decodeURIComponent(j[1])
}}}return l
}return f
}(adsk.referrerPersonalization||{}));
if(!CQ_Analytics.DemandBaseMgr){CQ_Analytics.DemandBaseMgr=function(){this.data=null;
this.initialData={};
this.copyObject=function(c){var b={};
for(var a in c){b[a]=c[a]
}return b
...
CQ_Analytics.DemandBaseMgr.prototype=new CQ_Analytics.PersistedJSONStore();
CQ_Analytics.DemandBaseMgr.prototype.STOREKEY="DEMANDBASE";
CQ_Analytics.DemandBaseMgr.prototype.STORENAME="demandbase";
CQ_Analytics.DemandBaseMgr.prototype.clear=function(){this.data=null;
this.initialData={}
};
CQ_Analytics.DemandBaseMgr.prototype.parsePropertyList=function(a){var d={};
var b=a.split(",");
for(var c in b){if(b.hasOwnProperty(c)){var e=b[c].split("=");
if(e.length==2){d[e[0]]=e[1]
}}}return d
};
CQ_Analytics.DemandBaseMgr.prototype.init=function(c){var a=new CQ_Analytics.SessionPersistence({container:"ClientContext"});
var b=a.get(this.getStoreKey());
if(b!=undefined){this.setProperties(this.parsePropertyList(b))
}if(c){if((this.data&&this.data.hasOwnProperty("country_code")&&c.hasOwnProperty("country_code")&&this.data.country_code!=c.country_code)||((!this.data||!this.data.hasOwnProperty("country_code"))&&c.hasOwnProperty("country_code"))||(this.data&&this.data.hasOwnProperty("country_code")&&!c.hasOwnProperty("country_code"))){setTimeout(function(){adsk.clientContext.getStore("demandbase").fireEvent("countryupdate")
},10)
}this.setProperties(c)
}this.initialData=this.copyObject(this.data);
this.persist();
this.initialized=true;
this.fireEvent("initialize",this)
};
CQ_Analytics.DemandBaseMgr.prototype.setProperties=function(a){this.data={};
for(var b in a){this.data[b]=a[b]
}this.fireEvent("update");
return true
};
CQ_Analytics.DemandBaseMgr.prototype.getDataFromDemandBase=function(){if(analytics.checks.demandbaseReady){this.setProperties(analytics._core.apis.DemandBase);
return true
}else{return false
}};
CQ_Analytics.DemandBaseMgr=new CQ_Analytics.DemandBaseMgr();
CQ_Analytics.CCM.addListener("configloaded",function(){CQ_Analytics.CCM.register(this)
},CQ_Analytics.DemandBaseMgr)
}window.adsk=window.adsk||{};
adsk.clientContext=(function(f){var a=false;
if(typeof localStorage==="object"){try{localStorage.setItem("localStorage",1);
localStorage.removeItem("localStorage");
a=true
}catch(d){Storage.prototype._setItem=Storage.prototype.setItem;
Storage.prototype.setItem=function(){}
}}f.hasLocalStorage=function(){return a
};
var b={};
var c="segments";
f.Store=function(){};
if(a){f.Store.getInstance=function(g){var e=new f.Store();
e.cqStore=CQ_Analytics.PersistedJSONStore.registerNewInstance(g);
return e
};
f.Store.prototype.getProperty=function(g,e){if(this.cqStore&&this.cqStore.data){return this.cqStore.getProperty(g,e)
}return""
};
f.Store.prototype.setProperty=function(e,g){this.cqStore.setProperty(e,g)
};
f.Store.prototype.removeProperty=function(e){this.cqStore.removeProperty(e)
};
f.Store.prototype.getPropertyNames=function(e){return this.cqStore.getPropertyNames(e)
};
f.Store.prototype.getJson=function(){if(typeof this.cqStore.getJSON==="function"){return this.cqStore.getJSON()
}else{if(typeof this.cqStore.getData==="function"){return this.cqStore.getData()
}else{return{}
}}};
f.getStore=function(h){if(b[h]===undefined){if(CQ_Analytics&&CQ_Analytics.StoreRegistry){var e=CQ_Analytics.StoreRegistry.getStore(h)
}if(e===undefined){b[h]=f.Store.getInstance(h)
}else{var g=new f.Store();
g.cqStore=e;
b[h]=g
}}return b[h]
};
f.removeStore=function(g){if(b[g]!==undefined){var e=b[g];
e.cqStore.reset();
delete b[g];
b[g]=undefined;
e.cqStore=undefined;
...
f.Store.prototype.getResolved=function(){var e=f.getStore(c);
if(typeof e.cqStore.getResolved==="function"){return e.cqStore.getResolved()
}return[]
};
f.Store.prototype.addListener=function(g,e){this.cqStore.addListener(g,e)
};
f.Store.prototype.removeListener=function(g,e){this.cqStore.removeListener(g,e)
};
f.Store.prototype.fireEvent=function(e){this.cqStore.fireEvent(e,this.cqStore)
}
}else{f.getStore=function(g){var e=new function(){};
e.getProperty=function(h){if(typeof h!=="undefined"&&(h=="location"||h=="country_code")){return"us"
}else{return""
}};
e.setProperty=function(h,j){};
e.removeProperty=function(h){};
e.getPropertyNames=function(){return
};
e.getJson=function(){return{}
};
return e
};
f.removeStore=function(e){}
}return f
}(adsk.clientContext||{}));
adsk.pblDisabled=true;
(function(){var a=CQ_Analytics.PersistedJSONStore.registerNewInstance("usersetlocation",{});
if(typeof adsk!=="undefined"&&adsk.pblDisabled===true&&$(document.documentElement).data("adsk-locale")==="en-US"){a.setProperty("location","US")
}})();
window.adsk=window.adsk||{};
adsk.profileCleaner=(function(f){var f=adsk.profileCleaner||{};
f.timemoutMinutes=10;
f.PROFILE_KEY="profile";
f.STORAGE_KEY=(window.CQ_Analytics!==undefined&&window.CQ_Analytics.UserProfileMgr!==undefined&&window.CQ_Analytics.UserProfileMgr.STORENAME!==undefined&&window.CQ_Analytics.UserProfileMgr.STORENAME.length>2)?window.CQ_Analytics.UserProfileMgr.STORENAME:"userprofile";
f.MAIN_OXYGEN_URL="/services/adsk/c/oxygen/tool";
f.EDU_EXTENTION="edu";
f.AU_EXTENTION="au";
f.OXYGEN_PROFILE_URL=".do/profile?ts=";
f.EDU_DOMAIN_UNIQUE="/education";
f.AU_DOMAIN_UNIQUE="au.autodesk.com";
f.ORG_DOMAIN_UNIQUE="autodesk.org";
f.STORE_DOMAIN_UNIQUE="/store";
f.AUTODESK_DOT_COM="com";
f.AUTODESK_UNIVERSITY="au";
f.AUTODESK_EDUCATION="edu";
f.AUTODESK_DOT_ORG="org";
f.AUTODESK_ESTORE="store";
f.userProfileCleaner=function(){var j;
var h=(new Date).getTime();
if(window.sessionStorage!==undefined&&sessionStorage.getItem(adsk.profileCleaner.PROFILE_KEY)!=="undefined"&&sessionStorage.getItem(adsk.profileCleaner.PROFILE_KEY)!==null){try{j=JSON.parse(sessionStorage.getItem(adsk.profileCleaner.PROFILE_KEY))
}catch(l){g()
}}if(j==undefined||j==null||j.fullname==undefined){if(typeof adsk!=="undefined"&&adsk.clientContext!==undefined){try{j=adsk.clientContext.getStore(adsk.profileCleaner.STORAGE_KEY).getJson();
if(j!==undefined&&j!==null&&j.fullname!==undefined){a();
j=undefined
}}catch(l){a()
}}}if(j!==null&&j!==undefined&&j.fullname!==undefined){if(window.sessionStorage.lastCheck==undefined){window.sessionStorage.lastCheck=h
}else{var k=window.sessionStorage.lastCheck;
if(((h-k)/60000)>adsk.profileCleaner.timemoutMinutes){jQuery.ajax({url:e()+new Date().valueOf(),dataType:"json",success:function(m){if(m&&m.fullname!=null){c(m)
}else{d()
}window.sessionStorage.lastCheck=h
}})
}else{window.sessionStorage.lastCheck=h
}}}else{}};
f.getMinutesBetweenCheck=function(){return f.timemoutMinutes
};
f.setMinutesBetweenCheck=function(h){f.timemoutMinutes=h
};
var e=function(){var h=adsk.profileCleaner.onWhichWebSiteAmI();
var j=(adsk&&adsk.pathCenter&&adsk.pathCenter.mainoxygenpath)?adsk.pathCenter.mainoxygenpath:adsk.profileCleaner.MAIN_OXYGEN_URL;
switch(h){case adsk.profileCleaner.AUTODESK_UNIVERSITY:j+=adsk.profileCleaner.AU_EXTENTION;
break;
case adsk.profileCleaner.AUTODESK_EDUCATION:j+=adsk.profileCleaner.EDU_EXTENTION;
break;
default:}j+=adsk.profileCleaner.OXYGEN_PROFILE_URL;
return j
};
f.getCurrentUrl=function(){return window.location.href
};
f.getDigitalDataSite=function(){return window.digitalData.page.site
};
f.getDigitalDataPageName=function(){return window.digitalData.page.pageName
};
f.isDigitalDataPageNameMatchEducation=function(){if(window.digitalData!==undefined&&window.digitalData){if(window.digitalData.page!==undefined&&window.digitalData.page){if(adsk.profileCleaner.getDigitalDataPageName()!==undefined&&adsk.profileCleaner.getDigitalDataPageName().indexOf("education")!==-1){return true
}}}return false
};
var b=function(){var j;
var h;
if(window.digitalData!==undefined){if(window.digitalData.page!==undefined){j=adsk.profileCleaner.getDigitalDataSite();
if(j!==undefined){h=j.split(":");
if(h!==undefined&&h.length>0){return h[0]
}}else{if(adsk.profileCleaner.getDigitalDataPageName()!==undefined&&adsk.profileCleaner.getDigitalDataPageName().indexOf(adsk.profileCleaner.AUTODESK_UNIVERSITY)===0){h=adsk.profileCleaner.getDigitalDataPageName().split(":");
if(h!==undefined&&h.length>0){return h[0]
}}}}}return undefined
};
f.onWhichWebSiteAmI=function(){var j=adsk.profileCleaner.getCurrentUrl();
var h=b();
if(h!==undefined){switch(h){case"au":return adsk.profileCleaner.AUTODESK_UNIVERSITY;
case"org":return adsk.profileCleaner.AUTODESK_DOT_ORG;
case"adsk":if(adsk.profileCleaner.isDigitalDataPageNameMatchEducation()){return adsk.profileCleaner.AUTODESK_EDUCATION
}else{return adsk.profileCleaner.AUTODESK_DOT_COM
}default:break
}}if(j.indexOf(adsk.profileCleaner.EDU_DOMAIN_UNIQUE)!==-1){return adsk.profileCleaner.AUTODESK_EDUCATION
}else{if(j.indexOf(adsk.profileCleaner.AU_DOMAIN_UNIQUE)!==-1){return adsk.profileCleaner.AUTODESK_UNIVERSITY
}else{if(j.indexOf(adsk.profileCleaner.STORE_DOMAIN_UNIQUE)!==-1){return adsk.profileCleaner.AUTODESK_ESTORE
}else{if(j.indexOf(adsk.profileCleaner.ORG_DOMAIN_UNIQUE)!==-1){return adsk.profileCleaner.AUTODESK_DOT_ORG
}}}}return adsk.profileCleaner.AUTODESK_DOT_COM
};
var d=function(){a();
g()
};
var a=function(){if(typeof adsk!=="undefined"&&adsk.clientContext!==undefined){adsk.clientContext.removeStore(adsk.profileCleaner.STORAGE_KEY)
}};
var g=function(){if(window.sessionStorage!==undefined){window.sessionStorage.removeItem(adsk.profileCleaner.PROFILE_KEY);
if(window.sessionStorage.getItem(adsk.profileCleaner.PROFILE_KEY)==null){window.sessionStorage.setItem(adsk.profileCleaner.PROFILE_KEY,undefined)
}}};
var c=function(l){var k;
if(typeof adsk!=="undefined"&&adsk.clientContext){k=adsk.clientContext.getStore(adsk.profileCleaner.STORAGE_KEY);
if(typeof k!=="undefined"){for(var j in l){if(l.hasOwnProperty(j)){k.setProperty(j,l[j])
}}}}if(window.sessionStorage!==undefined){try{window.sessionStorage.setItem(adsk.profileCleaner.PROFILE_KEY,JSON.stringify(l))
}catch(h){}}};
return f
}(adsk.profileCleaner||{}));
adsk.profileCleaner.userProfileCleaner();
window.adsk=window.adsk||{};
adsk.userLocation=(function(d){var c="en-US";
var a="US";
var b={US:"en-US",GB:"en-GB",DE:"de-DE",FR:"fr-FR",IT:"it-IT",ES:"es-ES",EU:"en-EU",AU:"en-AU",SG:"en-SG",JP:"ja-JP",KR:"ko-KR",CA:"en-CA",BE:"nl-BE",CZ:"cs-CZ",DK:"da-DK",FI:"fi-FI",HU:"hu-HU",NL:"nl-NL",NO:"no-NO",PL:"pl-PL",PT:"pt-PT",SE:"sv-SE",AT:"de-AT",BG:"bg-BG",GR:"el-GR",IE:"en-IE",RO:"ro-RO",HR:"hr-HR",SK:"sk-SK",SI:"sl-SI",CH:"de-CH"};
d.getCountry=function(f){var e;
var g=$(document.documentElement).data("run-mode");
if(g&&g.indexOf("dc")>-1||g.indexOf("wd")>-1){if(typeof adsk!=="undefined"&&adsk.clientContext){e=adsk.clientContext.getStore("usersetlocation").getProperty("location")||adsk.clientContext.getStore("demandbase").getProperty("country_code")
}if(typeof e==="undefined"||e==null||e===""){e=f?"":a
}else{e=e.toUpperCase()
}}return e
};
d.getCountryNoDefault=function(){return this.getCountry(true)
};
d.getLocaleForCountry=function(e){return b[e]||c
};
d.isGlobalSite=function(){return $(document.documentElement).attr("lang")===c
};
return d
}(adsk.userLocation||{}));
window.adsk=window.adsk||{};
window.adsk.oxygen=window.adsk.oxygen||{};
adsk.oxygen.common=(function(a){a.MAIN_OXYGEN_URL="/services/adsk/c/oxygen/tool";
a.OXYGEN_REGISTER_URL=".do/register.resp";
a.OXYGEN_LOGIN_URL=".do/connect.resp";
a.OXYGEN_FRAME_REGISTER_URL=".do/register";
a.OXYGEN_FRAME_LOGIN_URL=".do/connect";
a.OXYGEN_FRAME_UPDATE_URL=".do/updateprofile";
a.OXYGEN_LANG_ARG="lang";
a.getCommonLoginPath=function(){return adsk.oxygen.common.MAIN_OXYGEN_URL+adsk.oxygen.common.OXYGEN_LOGIN_URL
};
a.getCommonLoginUrl=function(c){return c?location.protocol+"//"+location.host+adsk.oxygen.common.getCommonLoginPath()+"?"+adsk.oxygen.common.OXYGEN_LANG_ARG+"="+c:location.protocol+"//"+location.host+adsk.oxygen.common.getCommonLoginPath()
};
a.getCommonRegisterPath=function(){return adsk.oxygen.common.MAIN_OXYGEN_URL+adsk.oxygen.common.OXYGEN_REGISTER_URL
};
a.getCommonFrameLoginPath=function(){return adsk.oxygen.common.MAIN_OXYGEN_URL+adsk.oxygen.common.OXYGEN_FRAME_LOGIN_URL
};
a.getCommonFrameRegisterPath=function(){return adsk.oxygen.common.MAIN_OXYGEN_URL+adsk.oxygen.common.OXYGEN_FRAME_REGISTER_URL
};
a.getCommonFrameUpdatePath=function(){return adsk.oxygen.common.MAIN_OXYGEN_URL+adsk.oxygen.common.OXYGEN_FRAME_UPDATE_URL
};
a.getCommonRegisterUrl=function(c){return c?location.protocol+"//"+location.host+adsk.oxygen.common.getCommonRegisterPath()+"?"+adsk.oxygen.common.OXYGEN_LANG_ARG+"="+c:location.protocol+"//"+location.host+adsk.oxygen.common.getCommonRegisterPath()
};
a.getCommonFrameLoginUrl=function(c){return c?location.protocol+"//"+location.host+adsk.oxygen.common.getCommonFrameLoginPath()+"?"+adsk.oxygen.common.OXYGEN_LANG_ARG+"="+c:location.protocol+"//"+location.host+adsk.oxygen.common.getCommonFrameLoginPath()
};
a.getCommonFrameRegisterUrl=function(c){return c?location.protocol+"//"+location.host+adsk.oxygen.common.getCommonFrameRegisterPath()+"?"+adsk.oxygen.common.OXYGEN_LANG_ARG+"="+c:location.protocol+"//"+location.host+adsk.oxygen.common.getCommonFrameRegisterPath()
};
a.getCommonFrameUpdateUrl=function(c){return c?location.protocol+"//"+location.host+adsk.oxygen.common.getCommonFrameUpdatePath()+"?"+adsk.oxygen.common.OXYGEN_LANG_ARG+"="+c:location.protocol+"//"+location.host+adsk.oxygen.common.getCommonFrameUpdatePath()
};
a.amILoggedIn=function(){return adsk.oxygen.common.getVisitorEmail()&&adsk.oxygen.common.getVisitorEmail().length>4?true:false
};
a.getVisitorEmail=function(){var c=adsk.oxygen.common.getLoggedUserProfile();
return typeof c!=="undefined"&&c!==null?c.email:undefined
};
a.getVisitorName=function(){var c=adsk.oxygen.common.getLoggedUserProfile();
if(typeof c!=="undefined"){if(c.fullname&&c.fullname.length>0){return c.fullname
}else{return c.firstname&&c.lastname?c.firstname+" "+c.lastname:""
}}};
a.getLoggedUserProfile=function(){var c;
if(window.sessionStorage&&typeof window.sessionStorage.profile!=="undefined"){try{c=JSON.parse(window.sessionStorage.profile)
}catch(d){}}return c
};
var b=function(d){var f;
if(!/(autodesk\.((com|co)?(\.)?([a-z]{2})?)(:[0-9]*)?)$/.test(d.origin)){return
}if(/^userprofile::/.test(d.data)){var c=d.data.substr(13);
if(window.sessionStorage){sessionStorage.profile=c
}var g=document.createEvent("Event");
g.initEvent("authenticated",true,false);
window.dispatchEvent(g)
}};
a.setEventliteners=function(){if(window.addEventListener){window.addEventListener("message",b,false)
}else{if(window.attachEvent){window.attachEvent("onmessage",b,false)
}}};
return a
}(adsk.oxygen.common||{}));
adsk.oxygen.common.setEventliteners();